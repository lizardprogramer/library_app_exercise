...
<body class="mb-48">
  ...
  <main>
    ...
    <div class="bg-gray-50 border border-gray-200 p-10 rounded max-w-lg mx-auto mt-24">
      ...
      <form method="POST" action="http://localhost/LibraryApp/public/admin/authors" enctype="multipart/form-data">
        @csrf
        <div class="mb-6">
        ...
        <div class="mb-6">
          <label for="biography" class="inline-block text-lg mb-2">
            Author biography
          </label>
          <textarea class="border border-gray-200 rounded p-2 w-full" name="biography" rows="7"
            placeholder="He was born in...">{{old('biography')}}</textarea>
          @error('biography')
          <p class="text-red-500 text xs mt-1">{{$message}}</p>
          @enderror
        </div>
        ...
        <div class="mb-6">
          ...
          <a href="http://localhost/LibraryApp/public/" class="text-black ml-4"> Back </a>
        </div>
      </form>
    </div>
    <h1 class="text-2xl uppercase text-black mb-10 mt-10 ml-4 font-bold">
      AUTH<span class="text-black">ORS:</span>
    </h1>
    <table class="w-full table-auto rounded-sm">
      <tbody>
        @foreach ($authors as $author)
        <tr class="border-gray-300 bg-background text-center">
          <td
            class="px-4 py-8 border-t border-b border-gray-300 text-lg text-black font-bold hover:bg-laravel hover:text-white">
            <p>
              <a href="http://localhost/LibraryApp/public/authors/{{$author->id}}">{{$author->name}}</a>
            </p>
          </td>
          @endforeach
        </tr>
